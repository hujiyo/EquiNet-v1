...
## 项目简介
 ...
 - **最新版本采用二分类方案**：专注于预测股票是否会上涨（上涨2%才被标记为上涨），输出0-1之间的概率值，更符合实际交易需求。使用固定的31个测试文件和评估样本，确保评估的一致性和可重复性。
 - **预测性能良好**：在实验条件参数以及实验数据集下，模型总体准确率达到60%，最高可接近64%目标，测试集AUC最高可达75.1%。在预测为上涨的股票集中，股票上涨真实2%的概率高达55%，远超随机平均水平（40%-43%），体现了一定的预测能力。当然，最佳的效果必须采用最佳的参数配置。
